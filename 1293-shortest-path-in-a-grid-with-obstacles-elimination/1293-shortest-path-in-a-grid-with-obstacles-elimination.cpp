        ...
        for(step = 0; bfs.size(); step++) {
            for(int n = bfs.size(); n--;) {
                ...
                if(pos.first - 1 >= 0 && re - grid[pos.first - 1][pos.second] > max(-1, r[pos.first - 1][pos.second])) {
                    r[pos.first - 1][pos.second] = re - grid[pos.first - 1][pos.second];
                    bfs.push({{pos.first - 1, pos.second}, r[pos.first - 1][pos.second]});
                }
                if(pos.second - 1 >= 0 && re - grid[pos.first][pos.second - 1] > max(-1, r[pos.first][pos.second - 1])) {
                    r[pos.first][pos.second - 1] = re - grid[pos.first][pos.second - 1];
                    bfs.push({{pos.first, pos.second - 1}, r[pos.first][pos.second - 1]});
                }
